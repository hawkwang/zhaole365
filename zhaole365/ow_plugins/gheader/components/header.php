...
class GHEADER_CMP_Header extends OW_Component
{
    ...
    public function __construct( $groupId )
    {
        ...
        OW::getDocument()->addScript($urlStatic . 'gheader.min.js?' . GHEADER_Plugin::PLUGIN_BUILD);
        OW::getDocument()->addStyleSheet($urlStatic . 'gheader.min.css?' . GHEADER_Plugin::PLUGIN_BUILD);
    ...
    private function getGroupInfo()
    {
        ...
        $privacyParams = array('action' => GROUPS_BOL_Service::PRIVACY_ACTION_VIEW_MY_GROUPS, 'ownerId' => $this->group->userId, 'viewerId' => OW::getUser()->getId());
    ...
    public function getTabs()
    {
        ...
        $tabs = new GHEADER_CMP_Tabs();
                  
        $tab = new BASE_MenuItem();
        $tab->setKey('index');
        $tab->setLabel($language->text('gheader', 'tab_index'));
        ...
        $tab = new BASE_MenuItem();
        $tab->setKey('users');
        $tab->setLabel($language->text('gheader', 'tab_users'));
        $tab->setUrl(OW::getRouter()->urlForRoute('groups-user-list', array('groupId' => $this->groupId)));
        $tab->setIconClass('ow_ic_user');
        $tab->setOrder(2);
        $tabs->addElement($tab);
                
        $event = new BASE_CLASS_EventCollector("gheader.collect_tabs", array(
    ...
    public function getPemissions()
    {
        ...
        $credits = GHEADER_CLASS_CreditsBridge::getInstance()->credits;
