...
class ZLGHEADER_CLASS_NotificationsBridge
{

    private static $classInstance;

    const ACTION_COMMENT = 'zlgcover_comment';
    const ACTION_LIKE = 'zlgcover_like';
    const ACTION_ADD = 'zlgcover_add';

    const TYPE_LIKE = 'zlgcover_like';
    const TYPE_ADD = 'zlgcover_add';
    const TYPE_COMMENT = 'zlgcover_comment';
    ...
    public function onItemRender( OW_Event $event )
    {
        ...
        $js->jQueryEvent("#" . $uniqId, 'click',
                'OW.ajaxFloatBox("ZLGHEADER_CMP_CoverView", [e.data.groupId], {layout: "empty", top: 50 }); return false;',
        array('e'), array(
            'groupId' => $groupId
        ));
    ...
    public function onDublicate( OW_Event $event )
    {
        ...
        if ( $userCount == 2 )
        {
            $data['string']['key'] = $params['entityType'] == self::TYPE_COMMENT
                ? 'zlgheader+notifications_comment_2'
                : 'zlgheader+notifications_like_2';
        ...
        {
            $data['string']['key'] = $params['entityType'] == self::TYPE_COMMENT
                ? 'zlgheader+notifications_comment_many'
                : 'zlgheader+notifications_like_many';
    ...
    public function onComment( OW_Event $event )
    {
        ...
        $groupUrl = ZLGROUPS_BOL_Service::getInstance()->getGroupUrl($group);
        ...
        $notificationData = array(
            'string' => array(
                'key' => 'zlgheader+notifications_comment_1',
                'vars' => array(
    ...
    public function onLike( OW_Event $event )
    {
        ...
        $notificationData = array(
            'string' => array(
                'key' => 'zlgheader+notifications_like_1',
                'vars' => array(
                    'user' => '<a href="' . $users[0]['userUrl'] . '">' . $users[0]['userName'] . '</a>',
                    'group' => $groupEmbed
                )
    ...
    private function triggerAction( OW_Event $event, $add )
    {
        ...
        $notificationData = array(
            'string' => array(
                'key' => $add ? 'zlgheader+notifications_cover_add' : 'zlgheader+notifications_cover_change',
                'vars' => array(
    ...
    public function onCollectActions( BASE_CLASS_EventCollector $e )
    {
        ...
        if ( ZLGHEADER_CLASS_NewsfeedBridge::getInstance()->isActive() )
        {
            $e->add(array(
                'section' => 'zlgroups',
                'action' => self::ACTION_LIKE,
