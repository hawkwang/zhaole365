...
ZLGHEADER.CORE.ObjectRegistry = {};

ZLGHEADER.CORE.uniqId = function( prefix )
{
    prefix = prefix || '';
...
* Model
*/
ZLGHEADER.CORE.AjaxModel = function( rsp, delegate )
{
   this.rsp = rsp;
...
ZLGHEADER.CORE.AjaxModel.PROTO = function()
{
...
ZLGHEADER.CORE.AjaxModel.prototype = new ZLGHEADER.CORE.AjaxModel.PROTO();

ZLGHEADER.CORE.UploadModel = function( rsp, delegate )
{
    this.rsp = rsp;
    this.delegate = delegate;
    this.fakeIframe = null;
    this.uniqId = ZLGHEADER.CORE.uniqId('uploadModel');

    ZLGHEADER.CORE.ObjectRegistry[this.uniqId] = this;
};

ZLGHEADER.CORE.UploadModel.PROTO = function()
{
...
ZLGHEADER.CORE.UploadModel.prototype = new ZLGHEADER.CORE.UploadModel.PROTO();

ZLGHEADER.bindDrag = function($node, delegate)
{
    $node = $($node);
...
ZLGHEADER.CoverMenu = function(delegate)
{
...
ZLGHEADER.Cover = function( options, delegate )
{
    var userId = options.userId;
    var groupId = options.groupId;

    var launcher = new ZLGHEADER.CoverImage.nativeLauncher(options.cover.data);
    ZLGHEADER.CoverImage.setLauncher(launcher);

    $(document).on('click', '#uh-cover-image', function()
    {
        ZLGHEADER.CoverImage.show();
    });
    ...
    uploadModel = new ZLGHEADER.CORE.UploadModel(options.uploader, uploadDelegate);
    ...
    ajaxModel = new ZLGHEADER.CORE.AjaxModel(options.responder, ajaxDelegate);
    ...
    {
        if ( !confirm(OW.getLanguageText('zlgheader', 'delete_cover_confirmation')) )
        {
            return false;
        }
    ...
    toolbarDelegate.choose = function()
    {
        ...
        fb = OW.ajaxFloatBox('GHEADER_CMP_MyPhotos', [{windowHeight: winHeight}], {
            ...
            title: OW.getLanguageText('zlgheader', 'my_photos_title'),
            scope: {
    ...
    ZLGHEADER.CoverMenu(toolbarDelegate, coverData);
    ...
    ZLGHEADER.bindDrag($overlay, dragDelegate);
...
ZLGHEADER.Header = function( options )
{
    var userId = options.userId;
    var groupId = options.groupId;
    ...
    this.cover = new ZLGHEADER.Cover(options.cover,
    {
...
ZLGHEADER.CoverImage.nativeLauncher = function( cover )
{
    this.show = function()
    {
...
ZLGHEADER.CoverImage.photoLauncher = function( cover )
{
    this.show = function()
    {
...
ZLGHEADER.PhotoSelector = function( options, delegate )
{
