...
class ZLGHEADER_CTRL_Header extends OW_ActionController
{
    ...
    public function uploader()
    {
        ...
        $attachSelector = 'window.parent.ZLGHEADER.CORE.ObjectRegistry[' . json_encode($uniqId) . ']';
    ...
    private function validateFile( $file )
    {
        ...
        {
            $error = $language->text('zlgheader', 'upload_file_extension_is_not_allowed');
    ...
    private function validateImage( UTIL_Image $image, $canvasWidth, $canvasHeight )
    {
        ...
        {
            $error = $language->text('zlgheader', 'validation_error_image_size', array(
                'minWidth' => $minWidth,
                'minHeight' => $minHeight
            ));
    ...
    private function uploadCover( $file, $query )
    {
        ...
        $pluginfilesDir = OW::getPluginManager()->getPlugin('zlgheader')->getPluginFilesDir();
        ...
        $cover = $this->service->findCoverByGroupId($groupId, ZLGHEADER_BOL_Cover::STATUS_TMP);

        if ( $cover === null )
        {
            $cover = new ZLGHEADER_BOL_Cover();
        }

        $extension = UTIL_File::getExtension($file['name']);
        $cover->file = uniqid('cover-' . $groupId . '-') . '.' . $extension;
        $cover->groupId = $groupId;
        $cover->status = ZLGHEADER_BOL_Cover::STATUS_TMP;
    ...
    {
        $error = ZLGHEADER_CLASS_CreditsBridge::getInstance()->credits->getErrorMessage(ZLGHEADER_CLASS_Credits::ACTION_ADD);
    ...
    public function addFromPhotos( $query )
    {
        ...
        $sourcePath = ZLGHEADER_CLASS_PhotoBridge::getInstance()->pullPhoto($photoId);
    ...
    public function saveCover( $coverData )
    {
        ...
        $tmpCover = $this->service->findCoverByGroupId($groupId, ZLGHEADER_BOL_Cover::STATUS_TMP);
        $activeCover = $this->service->findCoverByGroupId($groupId, ZLGHEADER_BOL_Cover::STATUS_ACTIVE);
        $cover = $tmpCover;

        if ( $cover === null )
        {
            $eventName = ZLGHEADER_BOL_Service::EVENT_UPDATE;
            $cover = $activeCover;
        }
        else
        {
            if ($activeCover === null)
            {
                $eventName = ZLGHEADER_BOL_Service::EVENT_ADD;
            }
            else
            {
                $eventName = ZLGHEADER_BOL_Service::EVENT_CHANGE;
        ...
        $cover->status = ZLGHEADER_BOL_Cover::STATUS_ACTIVE;
        ...
        $message = OW::getLanguage()->text('zlgheader', 'cover_save_success');
    ...
    public function cancelChanges( $coverData )
    {
        ...
        $this->service->deleteCoverByGroupId($groupId, ZLGHEADER_BOL_Cover::STATUS_TMP);

        $cover = $this->service->findCoverByGroupId($groupId, ZLGHEADER_BOL_Cover::STATUS_ACTIVE);
    ...
    public function removeCover( $coverData )
    {
        ...
        $message = OW::getLanguage()->text('zlgheader', 'cover_remove_success');
    ...
    public function loadMorePhotos( $query )
    {
        ...
        $photos = ZLGHEADER_CLASS_PhotoBridge::getInstance()->findUserPhotos($userId, $start, $count);
        $photoCount = count($photos);

        $listFull = $photoCount < $count;

        $list = new ZLGHEADER_CMP_MyPhotoList($photos);
